...
export async function GET() {
  return NextResponse.json({
    ok: true,
    route: '/api/anchors/xrpl',
    anchorPayload: buildExamplePayloads().xrplAnchor,
  });
}
...
export async function POST(req: Request) {
  ...
  const anchorId = typeof body.anchorId === 'string' ? body.anchorId : `anchor-xrpl-${anchorPayload.merkleRoot.slice(0, 12)}`;
