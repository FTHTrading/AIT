...
export async function GET() {
  return NextResponse.json({
    ok: true,
    route: '/api/anchors/polygon',
    anchorPayload: buildExamplePayloads().polygonAnchor,
  });
}
...
export async function POST(req: Request) {
  ...
  const anchorId = typeof body.anchorId === 'string' ? body.anchorId : `anchor-polygon-${anchorPayload.merkleRoot.slice(0, 12)}`;
